...
lnSkrI1=zeros(nX,1);
 ...
 lnSlrI(1)=0;
    if mod(k,1)==0
        %If k is integer, takes every other k-value:
        for i = 1:nK-1
%             lnSkrI1(i,1)=Tf/(2*tau0*(lef(i)/2))*trapz(lnSurI(i:(2*i))/cos(asin((sigmaK(i)-X(i))*lambda*Tf/(2*tau0))))*k*DX;
           lnSkrI1(i,1)=Tf/(tau0*(lef(i)/2))*trapz(lnSurI(i:(2*i))/(sqrt(4-((sigmaK(i)-X(i))^2*lambda^2*Tf^2)/tau0^2)))*k*DX;
        end
    else
        %If k is not integer (interpolation required):   
        %Interpolates lnSlr for k*X (non-integer k):
